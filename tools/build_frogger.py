...
def emit_home_slot(x, filled=False):
    ...
    if filled:
        ...
        rect_imm(x + 1, HOME_Y + 1, HOME_W - 2, HOME_H - 2, BRIGHT_GREEN)
        plot_imm(x + 2, HOME_Y + 2, DARK_GREY)
        plot_imm(x + 3, HOME_Y + 2, DARK_GREY)
        plot_imm(x + 2, HOME_Y + 3, DARK_GREY)
        plot_imm(x + 3, HOME_Y + 3, DARK_GREY)
...
for i, x in enumerate(HOME_XS):
    ...
    skip_target = f'home_draw_target_{i}'
    a.ld_r_n('d', x)
    a.ld_r_n('e', HOME_Y)
    a.ld_r_n('b', HOME_W)
    a.ld_r_n('c', GREEN)
...
a.add_a_n(2)
a.cp_n(64)
a.jp('c', 'dcd_window_in')
a.sub_n(64)
a.label('dcd_window_in')
a.ld_r_r('d', 'a')
a.inc_r('e')
a.ld_r_n('c', CYAN)
a.call('plot')
