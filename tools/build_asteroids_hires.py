#!/usr/bin/env python3
"""
ASTEROIDS HI-RES — A simple Asteroids-style game for the Cromemco Dazzler + D+7A joystick.
Builds a CP/M .COM file (Z80 machine code, ORG 0x0100).

Display: 128×128 monochrome, 2K hires mode (white on black)
Framebuffer: 0x4000 (page 0x20), 2048 bytes
  4 quadrants of 512 bytes: TL, TR, BL, BR
  Each quadrant: 64×64 pixels, 16 bytes/row, 4×2 pixels per byte
...
SHIP_SHAPES = [
    # Dir 0 (N): V chevron pointing up
    [(0,-4), (0,-3), (-1,-2), (-1,-1), (-2,0), (-2,1), (-3,2), (0,-3), (1,-2), (1,-1), (2,0), (2,1), (3,2)],
    # Dir 1 (NE): V chevron pointing upper-right
    [(3,-3), (3,-2), (2,-1), (2,0), (1,1), (1,2), (0,3), (2,-3), (1,-2), (0,-2), (-1,-1), (-2,-1), (-3,0)],
    # Dir 2 (E): V chevron pointing right
    [(4,0), (3,0), (2,-1), (1,-1), (0,-2), (-1,-2), (-2,-3), (3,0), (2,1), (1,1), (0,2), (-1,2), (-2,3)],
    # Dir 3 (SE): V chevron pointing lower-right
    [(3,3), (3,2), (2,1), (2,0), (1,-1), (1,-2), (0,-3), (2,3), (1,2), (0,2), (-1,1), (-2,1), (-3,0)],
    # Dir 4 (S): V chevron pointing down
    [(0,4), (0,3), (-1,2), (-1,1), (-2,0), (-2,-1), (-3,-2), (0,3), (1,2), (1,1), (2,0), (2,-1), (3,-2)],
    # Dir 5 (SW): V chevron pointing lower-left
    [(-3,3), (-3,2), (-2,1), (-2,0), (-1,-1), (-1,-2), (0,-3), (-2,3), (-1,2), (0,2), (1,1), (2,1), (3,0)],
    # Dir 6 (W): V chevron pointing left
    [(-4,0), (-3,0), (-2,-1), (-1,-1), (0,-2), (1,-2), (2,-3), (-3,0), (-2,1), (-1,1), (0,2), (1,2), (2,3)],
    # Dir 7 (NW): V chevron pointing upper-left
    [(-3,-3), (-3,-2), (-2,-1), (-2,0), (-1,1), (-1,2), (0,3), (-2,-3), (-1,-2), (0,-2), (1,-1), (2,-1), (3,0)],
]
NUM_SHIP_POINTS = 13
...
store_imm('ship_x', 64)
store_imm('ship_y', 64)
store_imm('ship_old_x', 64)
store_imm('ship_old_y', 64)
...
for bi in range(2):
    ...
    for ai in range(8):
        ...
        emit_asteroid_hit_check('d', 'e', ai, miss, extra_radius=1)
...
for i in range(8):
    ...
    a.ld_a_lbl(ast_lbl(i, 'dx'))
    a.or_r('a')
    a.jr('nz', f'sl_dxok_{i}')
    store_imm(ast_lbl(i, 'dx'), 1)
    a.label(f'sl_dxok_{i}')
    # Ensure dy != 0
    a.ld_a_lbl(ast_lbl(i, 'dy'))
    a.or_r('a')
    a.jr('nz', f'sl_dyok_{i}')
    store_imm(ast_lbl(i, 'dy'), 1)
    a.label(f'sl_dyok_{i}')
    store_imm(ast_lbl(i, 'size'), 1)
    store_imm(ast_lbl(i, 'timer'), 0)
    a.ld_a_lbl('tmp_dy')
    a.call('flip_unit')
    a.or_r('a')
    a.jr('nz', f'sl_sdxok_{i}')
    a.ld_r_n('a', s8(-1))
    a.label(f'sl_sdxok_{i}')
    a.ld_lbl_a('spawn_dx')
    a.ld_a_lbl('tmp_dx')
    a.call('flip_unit')
    a.or_r('a')
    a.jr('nz', f'sl_sdyok_{i}')
    a.ld_r_n('a', s8(-1))
    a.label(f'sl_sdyok_{i}')
    a.ld_lbl_a('spawn_dy')
    ...
    copy_lbl(ast_lbl(i, 'x'), 'spawn_x')
    copy_lbl(ast_lbl(i, 'y'), 'spawn_y')
    # Child 1: swap and keep — if either axis is 0, force to 1
    copy_lbl('tmp_dy', ast_lbl(i, 'dx'))
    copy_lbl('tmp_dx', ast_lbl(i, 'dy'))
    # Ensure dx != 0
    a.ld_a_lbl(ast_lbl(i, 'dx'))
    a.or_r('a')
    a.jr('nz', f'sm_dxok_{i}')
    store_imm(ast_lbl(i, 'dx'), 1)
    a.label(f'sm_dxok_{i}')
    # Ensure dy != 0
    a.ld_a_lbl(ast_lbl(i, 'dy'))
    a.or_r('a')
    a.jr('nz', f'sm_dyok_{i}')
    store_imm(ast_lbl(i, 'dy'), 1)
    a.label(f'sm_dyok_{i}')
    store_imm(ast_lbl(i, 'size'), 0)
    store_imm(ast_lbl(i, 'timer'), 0)
    # Child 2: flip directions, also ensure non-zero
    a.ld_a_lbl('tmp_dy')
    a.call('flip_unit')
    a.or_r('a')
    a.jr('nz', f'sm_sdxok_{i}')
    a.ld_r_n('a', s8(-1))
    a.label(f'sm_sdxok_{i}')
    a.ld_lbl_a('spawn_dx')
    a.ld_a_lbl('tmp_dx')
    a.call('flip_unit')
    a.or_r('a')
    a.jr('nz', f'sm_sdyok_{i}')
    a.ld_r_n('a', s8(-1))
    a.label(f'sm_sdyok_{i}')
    a.ld_lbl_a('spawn_dy')
...
a.label('draw_bullet_sprite')
emit_offsets_shape(BULLET_POINTS)
a.ret()
...
emit_offsets_shape(AST_SMALL_POINTS)
a.ret()
...
# plot: pixel at D=x(0-127), E=y(0-127), color C (0=clear, nonzero=set). Clobbers A,B,HL
a.label('plot')
a.push('bc')
a.push('de')
...
a.label('ship_x'); a.db(64)
a.label('ship_y'); a.db(64)
a.label('ship_old_x'); a.db(64)
a.label('ship_old_y'); a.db(64)
...
# ─── Save ──────────────────────────────────────────────────────────────────────
out = os.path.join(os.path.dirname(__file__), '..', 'web', 'public', 'ASTEROIDSHI.COM')
a.save(out)
out2 = os.path.join(os.path.dirname(__file__), '..', 'games', 'ASTEROIDSHI.COM')
a.save(out2)
